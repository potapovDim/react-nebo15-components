...
describe('Table', () => {
  ...
  describe('props', () => {
    ...
    it('head', () => {
      expect(mount(<Table />).find(TableHead)).to.have.length(1);
      expect(mount(<Table head={false} />).find(TableHead)).to.have.length(0);
    });

    it('tbody', () => {
      expect(mount(<Table />).find('tbody')).to.have.length(1);
      expect(mount(<Table tbody={false} />).find('tbody')).to.have.length(0);
    });

    it('zebra', () => {
      expect(mount(<Table />).find(`.${styles.zebra}`)).to.have.length(1);
      expect(mount(<Table zebra={false} />).find(`.${styles.zebra}`)).to.have.length(0);
    });

    it('hovered', () => {
      expect(mount(<Table />).find(`.${styles.hovered}`)).to.have.length(1);
      expect(mount(<Table hovered={false} />).find(`.${styles.hovered}`)).to.have.length(0);
    });

    it('rowComponent', () => {
      const elem = mount(<Table
